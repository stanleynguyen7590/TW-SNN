...
class PoissonGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        out = torch.mul(torch.le(torch.rand_like(input), torch.abs(
            input)*1.0).float(), torch.sign(input))
        return out
...
class VGG_SNN_STDB(nn.Module):
    ...
    def threshold_update(self, scaling_factor=1.0, thresholds=[]):
        ...
        for pos in range(len(self.classifier)-1):
            if isinstance(self.classifier[pos], nn.Linear):
                if thresholds:
                    self.threshold[prev+pos] = torch.tensor(
                        thresholds.pop(0)*self.scaling_factor)
    ...
    def _make_layers(self, cfg):
        ...
        if self.vgg_name == 'VGG11' and self.dataset == 'CIFAR100':
            layers += [quanLinear(8192, 1024, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(1024, 1024, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(1024, self.labels, bias=False)]
        elif self.vgg_name == 'VGG5' and self.dataset != 'MNIST':
            layers += [quanLinear(512*4*4, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(4096, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(4096, self.labels, bias=False)]

        elif self.vgg_name != 'VGG5' and self.dataset != 'MNIST':
            layers += [quanLinear(512*2*2, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(4096, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            ...
            layers += [quanLinear(128*7*7, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(4096, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            ...
            layers += [quanLinear(512*1*1, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
            layers += [nn.Dropout(0.5)]
            layers += [quanLinear(4096, 4096, bias=False)]
            layers += [nn.ReLU(inplace=True)]
    ...
    def neuron_init(self, x):
        ...
        for l in range(len(self.features)):
            ...
            if isinstance(self.features[l], nn.Conv2d):
                self.mem[l] = torch.zeros(
                    self.batch_size, self.features[l].out_channels, self.width, self.height)

            elif isinstance(self.features[l], nn.Dropout):
                self.mask[l] = self.features[l](
        ...
        for l in range(len(self.classifier)):
            ...
            if isinstance(self.classifier[l], nn.Linear):
                self.mem[prev+l] = torch.zeros(self.batch_size,
                                               self.classifier[l].out_features)

            elif isinstance(self.classifier[l], nn.Dropout):
                self.mask[prev +
                          l] = self.classifier[l](torch.ones(self.mem[prev+l-2].shape))
        ...
        for t in range(self.timesteps):
            ...
            for l in range(len(self.classifier)-1):
                ...
                if isinstance(self.classifier[l], (nn.Linear)):
                    ...
                    self.spike[prev+l] = self.spike[prev +
                                                    l].masked_fill((out > 0.5), t-1)
                    # self.spike[prev+l] = self.spike[prev +
                    #                                 l].masked_fill(out.bool(), t-1)

                    self.mem[prev+l] = self.leak*self.mem[prev+l] + \
            ...
            if not find_max_mem:
                self.mem[prev+l+1] = self.mem[prev+l+1] + \
                    self.classifier[l+1](out_prev)
        if find_max_mem:
            return max_mem
