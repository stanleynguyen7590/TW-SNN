...
class PoissonGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        out = torch.mul(torch.le(torch.rand_like(input), torch.abs(
            input)*0.9).float(), torch.sign(input))
        return out
...
class BasicBlock(nn.Module):
    ...
    def forward(self, dic):
        ...
        spike[pos] = spike[pos].masked_fill((out > 0.5), t-1)
        ...
        spike[pos+1] = spike[pos+1].masked_fill((out > 0.5), t-1)
        ...
        mem[pos+1] = leak*mem[pos+1] + \
            self.residual[3](out_prev) + self.identity(inp) - rst
        # else:
...
class RESNET_SNN_STDB(nn.Module):
    ...
    def __init__(self, resnet_name, activation='Linear', labels=10, timesteps=75, leak=1.0, default_threshold=1.0, alpha=0.5, beta=0.035, dropout=0.2, dataset='CIFAR10'):
        ...
        self.layer1 = self._make_layer(
            block, 64, cfg[self.resnet_name][0], stride=1, dropout=self.dropout)
        self.layer2 = self._make_layer(
            block, 128, cfg[self.resnet_name][1], stride=2, dropout=self.dropout)
        self.layer3 = self._make_layer(
            block, 256, cfg[self.resnet_name][2], stride=2, dropout=self.dropout)
        self.layer4 = self._make_layer(
            block, 512, cfg[self.resnet_name][3], stride=2, dropout=self.dropout)
        #self.avgpool 		= nn.AvgPool2d(2)
        self.classifier = nn.Sequential(
            quanLinear(512*2*2, labels, bias=False)
        )

        self.layers = {1: self.layer1, 2: self.layer2,
    ...
    def threshold_update(self, scaling_factor=1.0, thresholds=[]):
        ...
        for pos in range(len(self.pre_process)):
            if isinstance(self.pre_process[pos], nn.Conv2d):
                if thresholds:
                    self.threshold[pos] = torch.tensor(
                        thresholds.pop(0)*self.scaling_factor)
    ...
    def neuron_init(self, x):
        ...
        for l in range(len(self.pre_process)):
            ...
            if isinstance(self.pre_process[l], nn.Conv2d):
                self.mem[l] = torch.zeros(
                    self.batch_size, self.pre_process[l].out_channels, self.width, self.height)

            elif isinstance(self.pre_process[l], nn.Dropout):
                self.mask[l] = self.pre_process[l](
                    torch.ones(self.mem[l-2].shape))
        ...
        for i in range(1, 5):
            ...
            for index in range(len(layer)):
                for l in range(len(layer[index].residual)):
                    if isinstance(layer[index].residual[l], nn.Conv2d):
                        self.mem[pos] = torch.zeros(
                            self.batch_size, layer[index].residual[l].out_channels, self.width, self.height)
                        pos = pos + 1
                    elif isinstance(layer[index].residual[l], nn.Dropout):
                        self.mask[pos-1] = layer[index].residual[l](
                            torch.ones(self.mem[pos-1].shape))
        ...
        self.mem[pos] = torch.zeros(
            self.batch_size, self.classifier[0].out_features)
        ...
        for t in range(self.timesteps):
            ...
            for l in range(len(self.pre_process)):
                ...
                if isinstance(self.pre_process[l], nn.Conv2d):
                    ...
                    self.spike[l] = self.spike[l].masked_fill((out > 0.5), t-1)
                    ...
                    self.mem[l] = self.leak*self.mem[l] + \
                        self.pre_process[l](out_prev) - rst
                    out_prev = out.clone()
            ...
            for i in range(1, 5):
                layer = self.layers[i]
                for index in range(len(layer)):
                    out_prev = layer[index]({'out_prev': out_prev.clone(), 'pos': pos, 'act_func': self.act_func, 'mem': self.mem,
                                            'spike': self.spike, 'mask': self.mask, 'threshold': self.threshold, 't': t, 'leak': self.leak})
                    pos = pos+2
